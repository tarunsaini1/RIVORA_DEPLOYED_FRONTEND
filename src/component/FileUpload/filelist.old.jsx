...
const FileListModal = ({ 
  ...
  const handleAnalyzeFile = async (file) => {
    try {
      ...
      // When using your API utility, you don't need to specify method, headers, etc.
      // The API utility should handle that already
      const response = await API.post(`/api/ai/analyze/${file._id}`);
      
      console.log('Analysis API response:', response);
      
      // With your API utility, the response is likely already parsed as JSON
      // No need to call .json() on it
      ...
      // Update the files array with new insights
      const updatedFiles = files.map(f => 
        f._id === file._id 
          ? { ...f, aiInsights: data.insights, isProcessed: true } 
          : f
      );
      
      // If you're managing files state in this component
      if (typeof setFiles === 'function') {
        setFiles(updatedFiles);
      }
      // If files state is managed by parent
      else if (typeof onFilesUpdate === 'function') {
